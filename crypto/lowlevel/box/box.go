...
Box uses Curve25519, XChaCha20 and Poly1305 to encrypt and authenticate
...
func Seal(out, message []byte, nonce [xchacha20poly1305.NonceSize]byte, peersPublicKey, privateKey [lilycurve25519.KeySize]byte) ([]byte, error) {
	sharedKey := Precompute(peersPublicKey, privateKey)
	cipher, err := xchacha20poly1305.New(sharedKey[:])
	if err != nil {
		return nil, err
	}
	return cipher.Seal(out, nonce[:], message, nil), nil
}

// SealAfterPrecomputation performs the same actions as Seal, but takes a
// shared key as generated by Precompute.
func SealAfterPrecomputation(out, message []byte, nonce [xchacha20poly1305.NonceSize]byte, sharedKey [xchacha20poly1305.KeySize]byte) ([]byte, error) {
	cipher, err := xchacha20poly1305.New(sharedKey[:])
	if err != nil {
		return nil, err
	}
...
func Open(out, ciphertext []byte, nonce [xchacha20poly1305.NonceSize]byte, peersPublicKey, privateKey [lilycurve25519.KeySize]byte) ([]byte, error) {
	sharedKey := Precompute(peersPublicKey, privateKey)
	cipher, err := xchacha20poly1305.New(sharedKey[:])
	if err != nil {
		return nil, err
	}
	return cipher.Open(out, nonce[:], ciphertext, nil)
}

// OpenAfterPrecomputation performs the same actions as Open, but takes a
// shared key as generated by Precompute.
func OpenAfterPrecomputation(out, ciphertext []byte, nonce [xchacha20poly1305.NonceSize]byte, sharedKey [xchacha20poly1305.KeySize]byte) ([]byte, error) {
	cipher, err := xchacha20poly1305.New(sharedKey[:])
	if err != nil {
		return nil, err
	}

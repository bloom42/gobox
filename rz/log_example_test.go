...
func setup() {
	...
	// and rz.Logger globals -- you would not normally need to do this
	rz.SetGlobalLogger(rz.New(rz.TimeFieldFormat(""), rz.TimestampFunc(func() time.Time {
...
func ExampleFatal() {
	...
	rz.Fatal(fmt.Sprintf("Cannot start %s", service), rz.Err(err), rz.String("service", service))
...
func Example() {
	...
	if *debug {
		logger := rz.GlobalLogger()
		defer func() {
			rz.SetGlobalLogger(logger)
		}()
		rz.SetGlobalLogger(rz.With(rz.Level(rz.DebugLevel)))
	}

	rz.Info("This message appears when log level set to Debug or Info")
